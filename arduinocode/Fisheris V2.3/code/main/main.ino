...
const String deviceVer="FHISERIS V2.3";
...
void setupRelay(){
  ...
  Serial.println("Look At the Light Bulb.......");
  writeLCD("Check Light Bulb",2,2);
  delay(2000);
  ...
  for (int i = 1; i <= 4; i++)
  {
    ...
    if (i % 2){
      ...
      writeLCD("Light : " +String((relayStat)?"ON":"OFF"),3,((20-("Light : " +String((relayStat)?"ON":"OFF")).length())/2));
    }
    else{
      relayStat=false;
      digitalWrite(RELAY,relayStat);
...
String serilizeJson(String deviceId=deviceId,float weight=weight,float temp=temp,float lux=lux,boolean relayStat=relayStat,location cordinate=cordinate){
  ...
  if(cordinate.latitude!=NULL || cordinate.longitude!=NULL){
    dump_data_location["latitude"] = cordinate.latitude;
    dump_data_location["longitude"] = cordinate.longitude;
  }
  else{
    dump_data_location["latitude"] = (char*)0;
    dump_data_location["longitude"] = (char*)0;
  }
...
GSMCommandOutput GSMCommand(String commmand,boolean printStr=true,int timeoutWaiting=500,int charLastReceivedTimeout=500){
  if (printStr!=false)
  {
      Serial.println(F(""));
      Serial.println(F("---------------------------------------------------------------------------------------------------------------------------"));
  ...
  while(true){
    ...
    if(millis()-startTime>timeoutWaiting && responded==false){break;}
    if(GSMCharPos>=sizeof(GSMCharReading)){break;}
  ...
  else if (result.indexOf("AT+CGNSPWR=1")!=-1 && result.indexOf("OK")!=-1)
  {
     output.status=true;
  }
  else if (result.indexOf("AT+CGNSINF")!=-1 && result.indexOf("+CGNSINF: 1,1,")!=-1)
  {
     output.status=true;
  }
...
void setupATComToSend(){
  ...
  GSMCommand("AT+HTTPPARA=\"URL\",\""+postBypassUrl+"\"",true,3000,1000);
...
void setupGSM(){
  ...
  while(true){
    ...
    SIM808.println("AT");
    delay(300);
    String result=SIM808.readString();
...
location getCordinate(){
  GSMCommandOutput dataLocation = GSMCommand("AT+CGNSINF",false);
  if(dataLocation.status){
    String fetchLocation=dataLocation.outputCom;
    int trash1=fetchLocation.indexOf("+CGNSINF: 1,1,")+13;
    int trash2=fetchLocation.indexOf(",",trash1+1);
    int latPos=fetchLocation.indexOf(",",trash2+1);
    int longPos=fetchLocation.indexOf(",",latPos+1);
    cordinate.latitude=fetchLocation.substring(trash2+1,latPos).toFloat();
    cordinate.longitude=fetchLocation.substring(latPos+1,longPos).toFloat();
    // Serial.println(fetchLocation);
    // Serial.println(fetchLocation.substring(latPos+1,longPos));
  }
  else{
    cordinate.latitude=NULL;
    cordinate.longitude=NULL;
  }
  return cordinate;
}
...
void postReq(String content){
  ...
  GSMCommand(content);
  GSMCommandOutput checkDataSent = GSMCommand("AT+HTTPACTION=1",true,8000,7000);
  clearLCD(0,0,0,1);
  ...
  GSMCommand("AT+HTTPREAD",true,3000,1000);
}
...
int getSimSignal(){
  ...
  // Serial.println("signal:"+(signal.substring(signal.indexOf('+CSQ:')+2, signal.indexOf(','))));
  int safeVar=(signal.substring(signal.indexOf('+CSQ:')+2, signal.indexOf(','))).toInt();
  if (!safeVar) {
    return 0;
  }
  simSignal=safeVar;
...
void updateMainDisplay(){
  ...
  location fltLocation=getCordinate();
  ...
  // String strTmlLatLong;
  // location fltLocation=getCordinate();
  if(fltLocation.latitude==NULL || fltLocation.longitude==NULL){
    strTmlLatLong="N:NaN E:NaN";
  }
  else{
    strTmlLatLong="N:"+String(fltLocation.latitude,3)+" E:"+String(fltLocation.longitude,3);
  }
  ...
  writeLCD(strTmlRelayStat.substring(0, 5),1,10);
  ...
  clearLCD(0,0,0,1);
  writeLCD(strTmlLatLong,3,(20-strTmlLatLong.length())/2);
  Serial.println(F(""));
}
...
{
  Serial.begin(9600);
  Serial.println(deviceVer+" Begin....");
  
  setupLCD();//setup lcd
  // delay(1000);
  
  writeLCD(deviceVer,0,(20-deviceVer.length()+1)/2);//row,collum
  // delay(1000);
